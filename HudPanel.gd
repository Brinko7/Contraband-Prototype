...
func _draw():
	...
	draw_line(Vector2(x0, 50), Vector2(W - 4, 50), Color(1.0, 1.0, 1.0, 0.07), 1.0)
	...
	draw_line(Vector2(x0, 76), Vector2(W - 4, 76), Color(1.0, 1.0, 1.0, 0.07), 1.0)

	# ── Row 4: loot indicator ─────────────────────────────────────────────────
	var has_loot: bool = get_meta("has_loot", false)
	if has_loot:
		draw_rect(Rect2(x0, 79, 72, 12), Color(0.90, 0.70, 0.10, 0.22), true)
		draw_string(font, Vector2(x0 + 4, 88), "✓  LOOT SECURED",
			HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(0.95, 0.80, 0.15, 0.95))
	else:
		draw_string(font, Vector2(x0, 88), "LOOT:  ─ ─ ─",
